	...
	<div class="main__container">
		...
		<section class="product__con__wrap">
			...
			<c:forEach var="items" items="${itemss}">
				<div id="product__con" onClick="location.href ='/monami/detail?cmd=detail&id=${items.id}'" style="background:url('${items.imgUrl}') no-repeat; background-size:contain; cursor:pointer;">
		...
		function inputItem(items) {
			...
			var string = // 다른 곳 java파일에 ""안에 붙여넣으면 자동으로 string화 		
				"			<div class=\"product__con\" onClick=\"location.href ='/monami/detail?cmd=detail&id="+items.id+"'\" style=\"background:url('"+items.imgUrl+"') no-repeat; background-size:contain; cursor:pointer\">\r\n" + 
				"				<a href=\"/monami/detail?cmd=detail&id="+items.id+"\">" +
				"				<span class=\"info\">\r\n" + 
				"					<span class=\"txt_brand\">Monami / 모나미</span>\r\n" + 
				"					<span class=\"txt-ti\">"+items.name+"</span>\r\n" + 
